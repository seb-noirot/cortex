...
class CortexEntityLinksTest {
    ...
    fun admin_urls_by_type() {
        val base = "https://cortex.sportradar.ag"
        assertEquals(
            "https://cortex.sportradar.ag/admin/service/en123?catalogPageSlug=services",
            CortexEntityLinks.adminUrl(base, "service", "en123")
        )
        assertEquals(
            "https://cortex.sportradar.ag/admin/teams/t1?catalogPageSlug=teams",
            CortexEntityLinks.adminUrl(base, "team", "t1")
        )
        assertEquals(
            "https://cortex.sportradar.ag/admin/scorecard/s1?catalogPageSlug=scorecards",
            CortexEntityLinks.adminUrl(base, "scorecard", "s1")
        )
        // unknown type pluralizes with 's'
        assertEquals(
            "https://cortex.sportradar.ag/admin/project/p1?catalogPageSlug=projects",
            CortexEntityLinks.adminUrl(base, "project", "p1")
        )
        // group should map to teams UI
        assertEquals(
            "https://cortex.sportradar.ag/admin/teams/g1?catalogPageSlug=teams",
